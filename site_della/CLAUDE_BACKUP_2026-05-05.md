...
### E-mails transacionais — padrão visual

- Templates em `templates/emails/` e envio central em `apps/pedidos/emails.py`
- **Logo dos e-mails**: usar a versão escura `static/images/brand/d-logo-della.png` dentro de cabeçalho claro; evitar a logo branca para não sumir em clientes com fundo claro
- **Itens do pedido nos e-mails**: `nome_produto` pode manter destaque, mas `cor/tamanho` (`variacao_desc`) deve ficar menor e mais discreto que o nome principal
- **Assuntos de e-mail**: evitar traços longos (`—`) e preferir frases contínuas mais naturais
- Após mudar templates/CSS/assuntos de e-mail: `collectstatic` + HUP no gunicorn
...
| `30 * * * *` | `rastrear_pedidos_correios` — consulta API Correios para pedidos `enviado` com código de rastreio |
...
### Correios CWS (rastreamento automático)

- Portal: `https://cws.correios.com.br` — usuário = CNPJ `29049870000137`
- Autenticação: `POST https://api.correios.com.br/token/v1/autentica` com `Authorization: Basic base64(CNPJ:codigo_acesso)` → JWT com ~1h de validade
- JWT cacheado em Django cache (`correios_jwt_token`) para evitar reautenticação a cada cron
- Rastreio: `GET /sro/v1/objetos?codigosObjetos={codigo}&tipo=T&resultado=U&linguagem=pt`
- Credenciais: `CORREIOS_USUARIO` e `CORREIOS_CODIGO_ACESSO` no `.env` (código gerado no portal CWS, validade indeterminada)
- Serviço: `apps/pedidos/services/correios.py`
- **Cron a cada 1h (`:30`)**: `rastrear_pedidos_correios` — dois grupos:
  - **Grupo 1** (`pagamento_confirmado`/`em_separacao` com rastreio): detecta "objeto postado" → muda para `enviado` automaticamente
  - **Grupo 2** (`enviado` com rastreio): detecta "saiu para entrega" → e-mail; detecta "entregue" → muda status + e-mail
- **Fluxo 100% automático** (sem clique manual no admin): Bling webhook salva rastreio → cron detecta postagem → muda para `enviado` → cron detecta saiu/entregue → e-mails
- **"Saiu para entrega"** → e-mail `saiu_para_entrega.html` + `correios_email_saiu_entrega=True` (evita duplicata)
- **"Entregue"** → status muda para `entregue` + e-mail `entregue_avaliacao.html` + `HistoricoPedido`
- Detecção (`detectar_evento`): "postado" (CO/PO), "saiu para entrega" (OUT/RO), "entregue ao destinatário" (BDE) — por descrição case-insensitive + fallback por código
- **Admin "→ Enviado"**: ainda disponível para casos onde o webhook Bling não trouxe o rastreio ou postagem manual. NÃO envia e-mail ao clicar
- Fallback: `marcar_entrega_automatica` (7 dias após envio) continua ativo caso Correios não atualize o rastreio
- Testar: `python manage.py rastrear_pedidos_correios --dry-run --settings=core.settings.production`
- Log: `logs/rastreio_correios.log`

### E-mails de teste

```bash
python manage.py enviar_emails_teste --email SEU@EMAIL --settings=core.settings.production
...
# Com pedido real:
python manage.py enviar_emails_teste --email SEU@EMAIL --pedido 2026-0001 --settings=core.settings.production
```
...
| Upload limite | Nginx: `client_max_body_size 50M` global; 500M na rota ZIP. Django admin: `MAX_UPLOAD_SIZE_MB = 15` (fotos de produto até 15 MB). Site público (futuros uploads de cliente): manter 15 MB ou menos |
...
## Bugs corrigidos (sessão 2026-05-05)

- **Fotos JPEG somem do bloco de cor após salvar**: causa raiz = `MAX_UPLOAD_SIZE_MB = 5` — fotos profissionais de câmera/celular facilmente excedem 5 MB. `validate_image_upload` lançava `ValidationError`, mas o erro ficava completamente invisível porque `#imagens-group .tabular.inline-related { display: none !important; }` esconde toda a tabela inline, incluindo os erros. O usuário via o preview sumir (blob URL não sobrevive ao reload) sem nenhuma mensagem. Correção em três camadas:
  1. `MAX_UPLOAD_SIZE_MB` elevado de 5 → **15 MB** em `core/settings/base.py`
  2. `produto_admin_por_cor.js`: `validateFiles(files)` faz validação client-side de extensão + tamanho **antes** de atribuir ao row; `showUploadErrors(erros)` renderiza banner vermelho visível dentro de `#imagens-group`
  3. `produto_admin_por_cor.js`: `checkHiddenInlineErrors()` lê qualquer `.errorlist` dentro da tabela inline oculta e repropaga no banner visível — protege contra futuros erros server-side que ficariam escondidos
  - Banner estilizado com `.della-upload-error-banner` em `della_admin.css`

## Bugs corrigidos (sessão 2026-05-04 — parte 2)

- **Zoom ativava sobre as setas de navegação da galeria**: `mousemove` agora verifica `e.target.closest('.galeria-nav')` e desativa o zoom quando o cursor está sobre `.galeria-nav-prev` ou `.galeria-nav-next`. `mouseenter` igualmente ignorado sobre as setas.
- **Scroll bloqueado ao arrastar fotos no admin de produto**: drag nativo do HTML bloqueia scroll do mouse. Resolvido com: (1) `onWheelDuringDrag` — passa o `deltaY` para `window.scrollBy` durante o drag; (2) `autoScrollStep` em `requestAnimationFrame` — scrolla automaticamente quando o cursor se aproxima das bordas superior/inferior da viewport (`SCROLL_ZONE = 120px`). Ambos iniciam no `dragstart` e param no `dragend`.
...
- **Zoom na galeria: NÃO ativar quando cursor estiver sobre `.galeria-nav`** — verificar `e.target.closest('.galeria-nav')` no `mousemove` e `mouseenter`. As setas ficam dentro de `.galeria-principal` então o evento bubbles até o container; sem esse check o zoom fica ativo sobre os botões
- **Scroll durante drag no admin: usar `wheel` + `requestAnimationFrame`** — o drag nativo bloqueia scroll. `onWheelDuringDrag` com `window.scrollBy(0, e.deltaY)` libera a roda. `autoScrollStep` via rAF auto-scrolla quando cursor está dentro de `SCROLL_ZONE=120px` das bordas. Iniciar no `dragstart`, parar no `dragend`. NÃO tentar interceptar `wheel` com `preventDefault` (readonly em listeners passivos)
...
- **`MAX_UPLOAD_SIZE_MB = 15`** — limite do admin para fotos de produto. Fotos profissionais de câmera/celular facilmente passam de 5 MB; 15 MB é o teto prático. NÃO baixar para 5 MB — fazia fotos sumirem silenciosamente. Nginx permite 50 MB; o gargalo intencional é Django.
- **Erros do inline de fotos são invisíveis por padrão** — `#imagens-group .tabular.inline-related { display:none !important }` esconde toda a tabela, incluindo `.errorlist`. Qualquer nova validação server-side no `ProdutoImagem.clean()` que possa falhar em uso normal PRECISA de contrapartida client-side em `produto_admin_por_cor.js` (ver `validateFiles` + `checkHiddenInlineErrors`). Nunca adicionar validações que falham silenciosamente aqui.
...
| **Sistema de avaliações do cliente** | Criar página `/avaliacoes/` pública + formulário de avaliação + exibição no painel do cliente (`/conta/minha-conta/`). Model `Avaliacao` já existe em `apps/produtos/models.py`. Botão "Avaliar Minha Compra" no e-mail `entregue_avaliacao.html` já aponta para `{{ site_url }}/avaliacoes/`. |
| **Testar rastreio Correios com pedido real** | Após despachar um pedido real com código de rastreio, validar se o cron (`rastrear_pedidos_correios`) detecta os eventos "saiu para entrega" e "entregue" corretamente. Log em `logs/rastreio_correios.log`. Pode testar manualmente: `python manage.py rastrear_pedidos_correios --dry-run --settings=core.settings.production` |
...
| Item | |
|---|---|
| Layout e texto de todos os e-mails transacionais | ✅ Header com logo + "Porque ser única nunca sai de moda", footer com WhatsApp clicável + "Acessar minha conta". Ajustes pontuais por e-mail (cancelamento, saiu entrega, entregue). Validado em 2026-05-04. |
...
| Sanitize / magic bytes / limite upload | `apps/core_utils/sanitize.py` + `MAX_UPLOAD_SIZE_MB` em `core/settings/base.py` |
| Validação client-side de upload + banner de erro | `static/admin/js/produto_admin_por_cor.js` → `validateFiles`, `showUploadErrors`, `checkHiddenInlineErrors` |
...
| Rastreio Correios — serviço (token + consulta) | `apps/pedidos/services/correios.py` |
| Rastreio Correios — cron (detecta saiu/entregue) | `apps/pedidos/management/commands/rastrear_pedidos_correios.py` |
| E-mails transacionais de teste | `apps/pedidos/management/commands/enviar_emails_teste.py` |
| Template "saiu para entrega" | `templates/emails/saiu_para_entrega.html` |
